...
public class HeatMap {
    ...
    private static final int          HALFCIRCLEPICSIZE = 8;//32;
    ...
    public void createHeatMap(final float multiplier) {
        ...
        /*BufferedImage nImg=new BufferedImage(32, 32, circle.getType());
        Graphics2D graph=nImg.createGraphics();
        graph.drawImage(circle, 0, 0,32,32,null);
        graph.dispose();*/
        BufferedImage heatMap = new BufferedImage(maxXValue, maxYValue, 6);
        paintInColor(heatMap, new Color(255, 255, 255)/*Color.white*/);
        ...
        while (iterator.hasNext()) {
            ...
            addImage(heatMap, circle, /*opaque*/0.40f,
        ...
        addImage(output, heatMap, /*0.4f*/0.90f);
    ...
    private void remap(final BufferedImage heatMapBW) {
        ...
        final int gradientHight = colorGradiant.getHeight() -30;//- 1;
    ...
    public void remap2(final BufferedImage heatMapBW)
    {
        final BufferedImage colorGradiant = loadImage(System.getProperty("user.dir")
                                                                + File.separator
                                                                + "heatmap"
                                                                + File.separator
                                                                + "HeatmapPalette.bmp");
        final int width = heatMapBW.getWidth();
        final int height = heatMapBW.getHeight();
        final int gradientWidth = colorGradiant.getWidth() -1;//- 1;
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
